...
function main.toggle(scope)
    if state.get_enabled(state) then
        log.debug(scope, "nvim-kiro is now disabled!")

        return main.disable(scope)
    end

    log.debug(scope, "nvim-kiro is now enabled!")
...
function main.enable(scope)
    if state.get_enabled(state) then
        log.debug(scope, "nvim-kiro is already enabled")
    ...
    -- saves the state globally to `_G.NvimKiroPlugin.state`
    state.save(state)
end
...
function main.disable(scope)
    if not state.get_enabled(state) then
        log.debug(scope, "nvim-kiro is already disabled")
